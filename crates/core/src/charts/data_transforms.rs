...
impl DataTransforms {
    ...
    pub fn e1rm_to_chart_data(
        ...
        e1rm_data
            .iter()
            .filter(|(exercise, _, _)| exercise_filter.is_none_or(|filter| exercise == filter))
    ...
    pub fn volume_to_chart_data(
        ...
        for (exercise, week_start, sets, reps, tonnage) in volume_data {
            let body_part = body_part_map.get(exercise).unwrap_or(exercise).clone();
        ...
        chart_data.sort_by(|a, b| a["week"].as_str().cmp(&b["week"].as_str()));
    ...
    pub fn get_unique_exercises(data: &[(String, NaiveDate, f64)]) -> Vec<String> {
        let mut exercises: Vec<String> = data
            .iter()
...
mod tests {
    ...
    fn test_body_part_mapping() {
        ...
        assert_eq!(
            body_part_map.get("OHP.BB.STND"),
            Some(&"Shoulders".to_string())
        );
    }
    ...
    fn test_session_frequency_data() {
        ...
        let chart_data =
            DataTransforms::session_frequency_to_chart_data(&sessions, start_date, end_date);
