use super::vega_specs::{VegaLiteSpec, ChartTemplates, AxisEncoding, ColorEncoding};
...
impl BuiltinCharts {
    ...
    pub fn e1rm_over_time(
        ...
        let mut spec = ChartTemplates::line_chart()
            ...
            .y_axis("e1rm_kg", "quantitative", Some("Estimated 1RM (kg)".to_string()));
    ...
    pub fn weekly_volume_by_bodypart(
        ...
        let chart_title = title.unwrap_or_else(|| {
            format!("Weekly {} by Body Part", y_title)
        });
    ...
    pub fn session_frequency_heatmap(
        ...
        spec.encoding.y = Some(AxisEncoding {
            ...
            scale: Some(super::vega_specs::ScaleSpec {
                ...
                range: Some(vec![
                    json!("Mon"), json!("Tue"), json!("Wed"), json!("Thu"),
                    json!("Fri"), json!("Sat"), json!("Sun")
                ]),
...
impl ChartGenerator {
    ...
    pub fn generate_e1rm_chart(
        ...
        Ok(BuiltinCharts::e1rm_over_time(&e1rm_data, exercise_filter, None))
    }
    ...
    pub fn generate_all_charts(
        ...
        let volume_chart = BuiltinCharts::weekly_volume_by_bodypart(
            volume_data, None, VolumeMetric::TonnageKg, None
        );
        ...
        let heatmap = BuiltinCharts::session_frequency_heatmap(
            session_dates, date_range.0, date_range.1, None
        );
...
mod tests {
    ...
    fn test_volume_chart_generation() {
        let data = vec![
            ("BP.BB.FLAT".to_string(), NaiveDate::from_ymd_opt(2025, 1, 6).unwrap(), 3, 15, 450.0),
            ("SQ.BB.BACK".to_string(), NaiveDate::from_ymd_opt(2025, 1, 6).unwrap(), 4, 20, 800.0),
        ];
