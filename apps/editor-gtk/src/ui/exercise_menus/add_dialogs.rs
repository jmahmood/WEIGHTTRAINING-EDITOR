...
use crate::operations::plan_ops::{
    add_amrap_to_plan, add_percentage_set_to_plan, add_rpe_set_to_plan, add_time_based_to_plan,
};
...
use gtk4::prelude::*;
use gtk4::{
    Box as GtkBox, CheckButton, Dialog, DialogFlags, Entry, Expander, Label, Orientation,
...
pub fn show_add_rpe_set_dialog(state: Arc<Mutex<AppState>>) {
    ...
    let (
        ...
        alt_group_entry,
        _search_widget,
    ) = create_exercise_search_section(state.clone());
