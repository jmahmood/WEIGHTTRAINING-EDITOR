...
use gtk4::prelude::*;
use gtk4::{
    Box as GtkBox, Button, Entry, Expander, Label, ListView, NoSelection, Orientation,
    ScrolledWindow, SignalListItemFactory, SpinButton, StringList, StringObject,
};
...
pub fn create_exercise_list_view(
    exercises_model: StringList,
) -> (ListView, ScrolledWindow, SignalListItemFactory) {
...
pub fn rebuild_exercise_model<T, F>(
    exercises_model: &StringList,
    exercises_data: &std::rc::Rc<std::cell::RefCell<Vec<T>>>,
    format_fn: F,
) where
...
pub fn create_base_segment_section(
    ex: &str,
    label: Option<&str>,
...
#[allow(dead_code)]
pub fn create_sets_reps_section(
    sets: Option<u32>,
    min_reps: Option<u32>,
    max_reps: Option<u32>,
) -> (GtkBox, SpinButton, SpinButton, SpinButton) {
    let section = GtkBox::builder()
        .orientation(Orientation::Vertical)
        .spacing(8)
        .build();
...
#[allow(dead_code)]
pub fn create_training_params_section(
    rpe: Option<f64>,
    rest_sec: Option<u32>,
