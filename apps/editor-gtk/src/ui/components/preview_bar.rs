use crate::operations::plan::{copy_current_plan_to_device, save_current_plan};
...
pub fn create_preview_bar(state: Arc<Mutex<AppState>>, paths: Arc<AppPaths>) -> Box {
    ...
    glib::timeout_add_seconds_local(
        2,
        clone!(@strong state, @strong duration_label, @strong volume_label, @strong equipment_label => move || {
        ...
        for seg in &day.segments {
            match seg {
                Segment::Straight(s) => {
                    ...
                    total_sets += sets;
                    if let Some(weightlifting_core::RepsOrRange::Range(r)) = &s.reps {
                        total_reps += sets * r.min;
                    }
                    ex_codes.insert(s.base.ex.clone());
                }
                Segment::Rpe(r) => {
                    total_sets += r.sets;
                    if let Some(weightlifting_core::RepsOrRange::Range(x)) = &r.reps {
                        total_reps += r.sets * x.min;
                    }
                ...
                Segment::Superset(su) => {
                    for item in &su.items {
                        ...
                        if let Some(weightlifting_core::RepsOrRange::Range(r)) = &item.reps {
                            total_reps += item.sets * r.min;
                        }
                ...
                Segment::Scheme(sc) => {
                    for set in &sc.sets {
                        ...
                        total_sets += sets;
                        if let Some(weightlifting_core::RepsOrRange::Range(r)) = &set.reps {
                            total_reps += sets * r.min;
                        }
                    }
