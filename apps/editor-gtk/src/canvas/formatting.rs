...
pub fn format_segment(segment: &Segment, dict: Option<&HashMap<String, String>>) -> String {
    match segment {
        weightlifting_core::Segment::Straight(s) => format!("{}: {} sets × {}", 
            name_for(&s.base.ex, s.base.label.as_ref(), dict), 
            s.sets.unwrap_or(1),
            match &s.reps {
                Some(weightlifting_core::RepsOrRange::Range(r)) => format!("{}-{} reps", r.min, r.max),
                None => "unspecified reps".to_string(),
            }
        ),
        weightlifting_core::Segment::Amrap(a) => format!("{}: AMRAP {}-{} reps", 
            name_for(&a.base.ex, a.base.label.as_ref(), dict),
            a.base_reps,
            a.cap_reps
        ),
        weightlifting_core::Segment::Rpe(r) => format!("{}: {} sets @ RPE {}", 
            name_for(&r.base.ex, r.base.label.as_ref(), dict),
            r.sets,
            r.rpe
        ),
        weightlifting_core::Segment::Percentage(p) => format!("{}: {} prescriptions", 
        ...
        weightlifting_core::Segment::Superset(s) => {
            ...
            let exercises: Vec<String> = s.items.iter().map(|item| {
            ...
            format!("🔗 {} ({} rounds): {}", label, s.rounds, exercises.join(" + "))
        },
        weightlifting_core::Segment::Circuit(c) => {
            let exercises: Vec<String> = c.items.iter().map(|item| {
            ...
            format!("🔄 Circuit ({} rounds): {}", c.rounds, exercises.join(" → "))
        },
        weightlifting_core::Segment::GroupChoose(g) => format!("🎯 Choose {} from: {} options", g.pick, g.from.len()),
        ...
        weightlifting_core::Segment::Complex(c) => {
            ...
            let sequence_exercises = c.sequence.iter().map(|item| {
                if let Some(d) = dict { d.get(&item.ex).cloned().unwrap_or(item.ex.clone()) } else { item.ex.clone() }
            }).collect::<Vec<_>>().join(" → ");
            format!("🔀 Complex {} sets @ {}: {}", c.sets, anchor_description, sequence_exercises)
        },
        _ => format!("{:?}", segment),
    }
}
