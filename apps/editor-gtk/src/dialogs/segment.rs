...
use crate::ui::exercise_menus::edit_dialogs::dialog_components::{create_base_segment_section, create_sets_reps_section, create_training_params_section};
use crate::ui::exercise_menus::{show_add_amrap_dialog, show_add_circuit_dialog, show_add_complex_dialog,
...
use gtk4::{Dialog, DialogFlags, ResponseType, Box as GtkBox, Orientation, Label, SpinButton, DropDown, StringList, TextView, ScrolledWindow, Button, Expander};
use gtk4::prelude::*;
use glib::clone;
...
pub fn show_add_segment_to_day_dialog(state: Arc<Mutex<AppState>>, day_index: usize) {
    ...
    dialog.connect_response(clone!(@strong state, @strong dropdown => move |dialog, response| {
...
pub fn show_edit_comment_dialog(state: Arc<Mutex<AppState>>, comment: weightlifting_core::CommentSegment, day_index: usize, segment_index: usize) {
    let dialog = Dialog::with_buttons(
        Some("Edit Comment"),
        crate::ui::util::parent_for_dialog().as_ref(),
        DialogFlags::MODAL,
...
pub fn show_generic_segment_edit_dialog(
    ...
    let (ex, label, alt_group, sets, min_reps, max_reps, rpe, rest_sec) = match segment {
        weightlifting_core::Segment::Straight(s) => {
            let (min_r, max_r) = if let Some(weightlifting_core::RepsOrRange::Range(range)) = &s.reps {
            ...
            (s.base.ex.clone(), s.base.label.as_deref(), s.base.alt_group.as_deref(), 
             s.sets, min_r, max_r, s.rpe, rest)
        },
        weightlifting_core::Segment::Rpe(r) => {
            let (min_r, max_r) = if let Some(weightlifting_core::RepsOrRange::Range(range)) = &r.reps {
                (Some(range.min), Some(range.max))
            } else {
                (Some(8), Some(10))
            };
            ...
            (r.base.ex.clone(), r.base.label.as_deref(), r.base.alt_group.as_deref(),
             Some(r.sets), min_r, max_r, Some(r.rpe), rest)
        },
        weightlifting_core::Segment::Amrap(a) => {
            (a.base.ex.clone(), a.base.label.as_deref(), a.base.alt_group.as_deref(),
             None, Some(a.base_reps), Some(a.cap_reps), None, None)
        },
        weightlifting_core::Segment::Time(t) => {
            (t.base.ex.clone(), t.base.label.as_deref(), t.base.alt_group.as_deref(),
             None, None, None, t.rpe, None)
        },
    ...
    let (params_section, rpe_entry, rest_entry) = 
        create_training_params_section(rpe, rest_sec);
...
pub fn show_edit_percentage_dialog(
    ...
    if let weightlifting_core::Segment::Percentage(p) = segment {
        ...
        let (base_section, ex_entry, label_entry, alt_entry) = 
            create_base_segment_section(&p.base.ex, p.base.label.as_deref(), p.base.alt_group.as_deref());
        content.append(&base_section);
        alt_entry.set_sensitive(false);
        ...
        let groups_list = StringList::new(&["None"]);
        {
            ...
            if let Some(plan) = &state_lock.current_plan {
                ...
                let mut sel = 0u32;
                if let Some(ag) = p.base.alt_group.as_deref() { if let Some(pos) = names.iter().position(|n| n == ag) { sel = (pos + 1) as u32; } }
        ...
        dialog.connect_response(clone!(@strong state, @strong prescriptions_box => move |dialog, response| {
            if response == ResponseType::Accept {
                let ex_text = ex_entry.text().to_string();
                let label_text = if label_entry.text().is_empty() { 
                    None 
...
pub fn edit_segment_if_applicable(state: Arc<Mutex<AppState>>, segment: &weightlifting_core::Segment, day_index: usize, segment_index: usize) {
    match segment {
        ...
        | weightlifting_core::Segment::Time(_) => {
            ...
            show_exercise_editor_dialog(
                state.clone(),
                ExerciseEditorContext::Edit { day_index, segment_index, segment: segment.clone() }
            );
        },
