use crate::operations::groups::{
    create_group, delete_group, group_exists, update_group, validate_group_exercises,
};
...
use gtk4::prelude::*;
use gtk4::{
    Box as GtkBox, Button, ButtonsType, Dialog, DialogFlags, Entry, Label, ListBox, ListBoxRow,
    MessageDialog, MessageType, Orientation, Paned, ResponseType, ScrolledWindow, SelectionMode,
};
...
pub fn show_manage_exercise_groups_dialog_with_selection(
    ...
    let (right_panel, name_entry, members_list, search_widget, save_btn) =
        create_group_editor_panel(state.clone(), groups_list.clone(), dirty.clone());
        ...
        key.connect_key_pressed(move |_, keyval, _, _| {
            let navigate = |next: bool| {
                let (sel, neighbor) = if let Some(sel) = gl_ref.selected_row() {
                    ...
                    (
                        Some(sel),
                        neigh.and_then(|w| w.downcast::<ListBoxRow>().ok()),
                    )
                } else {
                ...
                if let Some(target_row) = neighbor.or(sel) {
                    if dirty.get() {
                        if !confirm_discard_for(&parent_dialog) {
                            return;
                        }
                        dirty.set(false);
                    }
...
fn create_group_editor_panel(
    ...
    Entry,
    ListBox,
    crate::ui::widgets::ExerciseSearchWidget,
    gtk4::Button,
) {
    let panel = GtkBox::builder()
        .orientation(Orientation::Vertical)
    ...
    delete_btn.connect_clicked(
        clone!(@strong state, @strong name_entry, @strong groups_list => move |_| {
                    ...
                    if let Some(sel) = ml.selected_row() {
                        ...
                        if let Some(child) = sel.child() {
                            if let Some(hbox) = child.downcast_ref::<GtkBox>() {
                                if let Some(code_lbl) =
                                    hbox.first_child().and_then(|c| c.downcast::<Label>().ok())
                                {
...
fn add_exercise_to_members_list(
    members_list: &ListBox,
    code: &str,
    name: &str,
    search_widget: Option<crate::ui::widgets::ExerciseSearchWidget>,
) {
    ...
    remove_btn.connect_clicked(
        clone!(@strong row, @strong members_list, @strong search_widget => move |_| {
...
fn save_current_group(
    state: Arc<Mutex<AppState>>,
    name_entry: &Entry,
    members_list: &ListBox,
    _groups_list: &ListBox,
) -> bool {
...
fn delete_current_group(state: Arc<Mutex<AppState>>, name_entry: &Entry, groups_list: &ListBox) {
    ...
    dialog.connect_response(
        clone!(@strong state, @strong group_name, @strong groups_list => move |dialog, response| {
...
fn show_new_group_dialog(state: Arc<Mutex<AppState>>, groups_list: &ListBox) {
    ...
    dialog.connect_response(
        clone!(@strong state, @strong groups_list => move |dialog, response| {
        ...
        if let Some(list_row) = row.downcast_ref::<ListBoxRow>() {
            if let Some(container) = list_row.child() {
                if let Some(container) = container.downcast_ref::<GtkBox>() {
                    if let Some(name_label) = container
                        .first_child()
        ...
        if let Some(list_row) = row.downcast_ref::<ListBoxRow>() {
            if let Some(container) = list_row.child() {
                if let Some(container) = container.downcast_ref::<GtkBox>() {
                    ...
                    let maybe_name = container
                        .first_child()
                        .and_then(|c| c.downcast::<Label>().ok());
                    let maybe_count = maybe_name
                        .as_ref()
                        .and_then(|_| container.last_child())
                        .and_then(|c| c.downcast::<Label>().ok());
