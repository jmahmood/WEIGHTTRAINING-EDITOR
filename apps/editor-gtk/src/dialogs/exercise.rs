...
use gtk4::{Dialog, DialogFlags, ResponseType, Box as GtkBox, Orientation, Label, SpinButton, Expander, Entry, EventControllerKey};
use gtk4::prelude::*;
use glib::clone;
use std::sync::{Arc, Mutex};

pub fn show_add_exercise_dialog(state: Arc<Mutex<AppState>>) {
    show_exercise_editor_dialog(state, ExerciseEditorContext::Create { day_index: None, initial: None });
}

#[allow(dead_code)]
pub fn show_add_exercise_dialog_with_day(state: Arc<Mutex<AppState>>, day_index: Option<usize>) {
    show_exercise_editor_dialog(state, ExerciseEditorContext::Create { day_index, initial: None });
}

#[derive(Clone)]
#[allow(dead_code)]
pub enum ExerciseEditorContext {
    Create { day_index: Option<usize>, initial: Option<weightlifting_core::Segment> },
    Edit { day_index: usize, segment_index: usize, segment: weightlifting_core::Segment },
}
...
pub fn show_exercise_editor_dialog(state: Arc<Mutex<AppState>>, ctx: ExerciseEditorContext) {
    let (dlg_title, _day_index) = match &ctx {
        ExerciseEditorContext::Create { day_index, .. } => (
            if let Some(idx) = day_index { format!("Add Exercise to Day {}", idx + 1) } else { "Add Exercise".to_string() },
            *day_index
        ),
        ExerciseEditorContext::Edit { day_index, .. } => ("Edit Exercise".to_string(), Some(*day_index)),
    };
    ...
    let (search_expander, group_search_expander, ex_entry, label_entry, alt_group_entry, search_widget, _group_search_widget) = create_exercise_search_section(state.clone(), current_alt_group);
    ...
    match &ctx {
        ExerciseEditorContext::Edit { segment, .. } => {
            use weightlifting_core::{Segment, RepsOrRange};
            match segment {
                Segment::Straight(s) => {
                    ...
                    if let Some(r) = s.rpe { rpe_entry.set_value(r); }
                    selected_summary.set_text(&format!("Current: {} ({})",
                        if let Some(name) = s.base.label.clone() { name } else { s.base.ex.clone() }, s.base.ex));
                }
                Segment::Rpe(r) => {
                    ...
                    if let Some(RepsOrRange::Range(rr)) = &r.reps { min_reps.set_value(rr.min as f64); max_reps.set_value(rr.max as f64); }
                    rpe_entry.set_value(r.rpe);
                    selected_summary.set_text(&format!("Current: {} ({})",
                        if let Some(name) = r.base.label.clone() { name } else { r.base.ex.clone() }, r.base.ex));
                }
                Segment::Amrap(a) => {
                    ...
                    selected_summary.set_text(&format!("Current: {} ({})",
                        if let Some(name) = a.base.label.clone() { name } else { a.base.ex.clone() }, a.base.ex));
                }
                Segment::Time(t) => {
                    ...
                    if let Some(r) = t.rpe { rpe_entry.set_value(r); }
                    selected_summary.set_text(&format!("Current: {} ({})",
                        if let Some(name) = t.base.label.clone() { name } else { t.base.ex.clone() }, t.base.ex));
                }
                _ => {}
            }
        }
...
fn create_exercise_search_section(state: Arc<Mutex<AppState>>, current_alt_group: Option<String>) -> (Expander, Expander, Entry, Entry, Entry, ExerciseSearchWidget, GroupSearchWidget) {
    ...
    (search_expander, group_search_expander, ex_entry, label_entry, alt_group_entry, search_widget, group_search_widget)
}
