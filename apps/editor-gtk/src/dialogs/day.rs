...
use gtk4::{Dialog, DialogFlags, ResponseType, Box as GtkBox, Orientation, Label, Entry, SpinButton};
use gtk4::prelude::*;
use glib::clone;
    ...
    let (day_number, current_label) = {
        ...
        if let Some(plan) = &app_state.current_plan {
            if day_index < plan.schedule.len() {
                (plan.schedule[day_index].day, plan.schedule[day_index].label.clone())
            } else {
...
fn create_day_dialog_content(state: Arc<Mutex<AppState>>) -> (GtkBox, SpinButton, Entry, SpinButton, Entry) {
    let content = GtkBox::builder()
        .orientation(Orientation::Vertical)
        .margin_start(20)
        .margin_end(20)
...
fn add_new_day_to_plan(
    ...
    if let Some(plan) = &mut app_state.current_plan {
        ...
        app_state.clear_selection();
        drop(app_state);
        println!("Created new plan and added Day {} - {}", day_number, day_label);
        update_canvas_content(state);
    }
}
