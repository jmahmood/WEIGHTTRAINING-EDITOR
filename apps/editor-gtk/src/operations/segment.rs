...
use weightlifting_core::{
    BaseSegment, CommentSegment, Day, GroupOptionalSegment, RepsOrRange, RepsRange, Segment,
    StraightSegment,
};
...
pub fn group_selected_segments(state: Arc<Mutex<AppState>>) {
    ...
    if app_state.selected_segments.len() < 2 {
        println!(
            "Need at least 2 segments to create a group. Current selection: {}",
...
pub fn ungroup_selected_segments(state: Arc<Mutex<AppState>>) {
    ...
    if !app_state.has_selection() {
        println!(
            "No segments selected. Select group segments first, then press U to ungroup them."
        );
        return;
    }
    ...
    println!(
        "Ungrouping {} selected segments (not fully implemented)",
        app_state.selected_segments.len()
    );
    app_state.clear_selection();
    app_state.mark_modified();
...
pub fn add_custom_exercise_to_plan(
    ...
    add_custom_exercise_to_day(
        state, target_day, ex_code, ex_label, alt_group, sets, min_reps, max_reps, rpe,
    );
}

pub fn add_custom_exercise_to_day(
    state: Arc<Mutex<AppState>>,
    day_index: usize,
    ex_code: String,
    ex_label: String,
    ...
    if let Some(plan) = &mut app_state.current_plan {
        ...
        let target_day_index = if day_index >= plan.schedule.len() {
            0
        } else {
            day_index
        };
...
pub fn add_custom_comment_to_day(
    state: Arc<Mutex<AppState>>,
    day_index: usize,
    text: String,
    icon: Option<String>,
...
pub fn remove_exercise_from_superset(
    state: Arc<Mutex<AppState>>,
    day_index: usize,
    segment_index: usize,
    ...
    if let Some(plan) = &mut app_state.current_plan {
        ...
        {
            if let weightlifting_core::Segment::Superset(ref mut superset) =
                &mut plan.schedule[day_index].segments[segment_index]
            {
...
pub fn remove_exercise_from_circuit(
    state: Arc<Mutex<AppState>>,
    day_index: usize,
    segment_index: usize,
    exercise_index: usize,
    ...
    if let Some(plan) = &mut app_state.current_plan {
        if day_index < plan.schedule.len()
            && segment_index < plan.schedule[day_index].segments.len()
        {
            if let weightlifting_core::Segment::Circuit(ref mut circuit) =
                &mut plan.schedule[day_index].segments[segment_index]
            {
    ...
    if app_state.current_plan.is_some() {
        ...
        if let Some(ref mut plan) = app_state.current_plan {
            if day_index < plan.schedule.len()
                && segment_index < plan.schedule[day_index].segments.len()
            {
                if let weightlifting_core::Segment::Comment(ref mut comment) =
                    &mut plan.schedule[day_index].segments[segment_index]
                {
                    comment.text = text;
                    comment.icon = icon;
    ...
    if app_state.current_plan.is_some() {
        ...
        if let Some(ref mut plan) = app_state.current_plan {
            if day_index < plan.schedule.len()
                && segment_index < plan.schedule[day_index].segments.len()
            {
                if let weightlifting_core::Segment::Straight(ref mut straight) =
                    &mut plan.schedule[day_index].segments[segment_index]
                {
                    straight.base.ex = ex;
    ...
    if app_state.current_plan.is_some() {
        ...
        if let Some(ref mut plan) = app_state.current_plan {
            if day_index < plan.schedule.len()
                && segment_index < plan.schedule[day_index].segments.len()
            {
                if let weightlifting_core::Segment::Rpe(ref mut rpe_seg) =
                    &mut plan.schedule[day_index].segments[segment_index]
                {
                    rpe_seg.base.ex = ex;
    ...
    if app_state.current_plan.is_some() {
        ...
        if let Some(ref mut plan) = app_state.current_plan {
            if day_index < plan.schedule.len()
                && segment_index < plan.schedule[day_index].segments.len()
            {
                if let weightlifting_core::Segment::Amrap(ref mut amrap) =
                    &mut plan.schedule[day_index].segments[segment_index]
                {
                    amrap.base.ex = ex;
    ...
    if app_state.current_plan.is_some() {
        ...
        if let Some(ref mut plan) = app_state.current_plan {
            if day_index < plan.schedule.len()
                && segment_index < plan.schedule[day_index].segments.len()
            {
                if let weightlifting_core::Segment::Time(ref mut time_seg) =
                    &mut plan.schedule[day_index].segments[segment_index]
                {
                    time_seg.base.ex = ex;
    ...
    if app_state.current_plan.is_some() {
        ...
        if let Some(ref mut plan) = app_state.current_plan {
            if day_index < plan.schedule.len()
                && segment_index < plan.schedule[day_index].segments.len()
            {
                if let weightlifting_core::Segment::Percentage(ref mut pct_seg) =
                    &mut plan.schedule[day_index].segments[segment_index]
                {
                    pct_seg.base.ex = ex;
